...
Single-user personal finance database with settings, categories,
transactions, savings goals, and recurring transactions.
"""
import json
import aiosqlite
...
async def find_category_by_keywords(text: str) -> tuple[dict | None, float]:
    ...
    Confidence: 0.0-1.0 where 1.0 = exact match.
    """
        ...
        for keyword in cat["keywords"]:
            ...
            if keyword_lower in text_lower:
                ...
                if f" {keyword_lower} " in f" {text_lower} ":
                    score += 0.3
                score = min(score + 0.4, 1.0)  # Base confidence boost
